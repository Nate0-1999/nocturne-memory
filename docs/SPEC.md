...
**Version 2.121** (2026-09-16) — WHITE, GRAY, BLACK (owner: 'Maybe there is a hard yes list and a grey list and a black list. Anything on the black list needs further approval but anything on the white or gray is good, the implementation agent should be extra careful on the gray.'): every packet works under three tiers. WHITE — its declared set: go. GRAY — every other product file, taken by function under v2.120's care (peers' pushed branches checked, only what the rows need, rebase before handoff, crossings listed): go, carefully. BLACK — needs a word from the owner or the gate before a line changes: a function a peer's pushed branch is live in; a protected row's behavior outside a flag-named corner; the law files (SPEC.md, AXIOMS.md, CLAUDE.md, AGENTS.md — the gate's pen, frozen copies excepted when exported); the release and CI workflows without the grant sentence; owner-side settings and credentials; a peer's evidence. The push bell marks black files as it marks protected rows. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).
...
| 155 | 2026-09-13 | v2.113 THE RULES LEDGER: every rule mapped to its enforcement (test/lint/canon/code/gate/none) with evidence; 19 testable rules lacked any check; the map found rule-vs-code conflicts (a top_k cap beside the threshold; spend reconciliation on a clock; a public tombstone PATCH; the ensemble term surviving in the glossary and ADR-012; the cube rule vs the modules ruling) and rule-vs-rule questions for the owner (deprecated memories deleted vs never hard-deleted). M3RC minted: the rules machine. GATE §5 amended | ACCEPTED |
| 156 | 2026-09-13 | v2.114 THE BOOT DIET: sized boot reading (charge + §0–§2 + board row + GATE §0 + glossary; SPEC by named section only; scouts read ledger-report.json, never the ledger source); motivated by measured re-reads after compaction (PLAN 20×, SPEC 13× in one session). PLAN §0 STEP 1 amended; M3RW charge amended | ACCEPTED |
| 157 | 2026-09-14 | v2.115 THE PROTECTED MARK: settled rows (walk PASS or owner-iterated) are data in the ledger; fix packets touch only flagged corners of them; impact lists and walks put them first; regression there is a stop. GATE §5 amended; M3FX bounded | ACCEPTED |
| 158 | 2026-09-15 | v2.116 THE PALACE PACKAGE IS nocturne-memory (owner ruling before the first publish; 'nocturne-spine' never published; PyPI pending publisher re-entered by the owner): package name, release workflow title, and the harness pin change; import/repo names untouched. M3NM minted with the registry-install proof (closes F087, F093) | ACCEPTED |
| 159 | 2026-09-15 | v2.117 NAMES MATCH: packages `nocturne-harness` + `nocturne-memory`, repositories named the same (owner renames nocturne-spine → nocturne-memory on GitHub; PyPI publisher entries re-entered); `nocturne` command unchanged; supersedes 073 (nocturne-ai) and 158's package-only rename. M3NM widened | ACCEPTED |
| 160 | 2026-09-15 | v2.118a MEMORIES-THEN-DROP IS THE DEFAULT compaction strategy (owner: 'memories then drop as the default'); the suite of SD-061 approved; M3CM approved and widened (strategies, policy, OpenRouter usage accounting; SD-062 rides) | ACCEPTED |
| 161 | 2026-09-15 | v2.118b THE SPRINT (owner: 'sprinting to complete all the rest of the features we have described by getting them fully spec'd and built asap'): every unfinished ledger row is owned by one packet; waves in PLAN §5.0; M3TH, M3MU, M3OR minted for the real gaps of the third walk; M3MI folded into M3MU; the fourth walk is the review-day scout | ACCEPTED |
| 162 | 2026-09-15 | v2.119 THE THREE 3D MODULES (farm, roots, palace in three.js; function-bound; look-law) + ROOTS OPACITY IS A LOOK, no value rule (OQ-17's selection-focus encoding stays a look choice). FL-123 moves from the Symphony packet to the visualization packet | ACCEPTED |
| 163 | 2026-09-16 | v2.120 THE SPLIT IS A NOTE, NOT A WALL (crossings by function + rebase; STOP only on a same-function collision) + THE PACKET RE-FREEZES (flag/ledger changes export the frozen copies in the same handoff). Six stops in one day were the wall, not the work; F112 and F111 resolved by this rule | ACCEPTED |
| 164 | 2026-09-16 | v2.121 WHITE, GRAY, BLACK (owner's tiers): the declared set, everything else by function with care, and a named black list that needs a word; bin/ledger's push bell prints [BLACK: needs a word] on law and workflow files | ACCEPTED |
...
**v2.112 (2026-09-12) narrative, folded from the header:** THE FEATURE LEDGER (owner: 'I want the features to be the thing that the scout checks every time'): notes/feature-ledger.md is the standing checklist — one row per feature carrying the owner's own words where it was aligned, my one-line label stating only what he said, its status (built · partial · not built · horizon · dropped · unverified), and the walk a scout runs. Every scout runs every built/partial row and reports PASS/FAIL per row with a screenshot; a feature packet is not DONE until its row has a walk the scout can run. Labels use GLOSSARY.md terms and add no constraint the owner did not state; rows resting on the constitution say so. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.113 (2026-09-13) narrative, folded from the header:** THE RULES LEDGER (owner: 'ask the same questions of all the concise rules to follow of the build before we mechanically implement both'): notes/rules-ledger.md — 169 rules, each with the owner's verbatim where it was aligned, a label stating only what he said, its kind (127 must · 28 ruling · 14 principle), and what enforces it today (102 test · 6 canon · 4 code · 20 gate-only · 37 none); 19 testable rules have no mechanical check and get one through M3RC (the rules machine). Rules and code that disagree go to the owner before enforcement. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.114 (2026-09-13) narrative, folded from the header:** THE BOOT DIET (owner: 'was there a reason the compaction was weird?'; the gate's finding: the boot reading is ~940 KB, the constitution alone 468 KB; the rules-machine session re-read PLAN 20× and SPEC 13× after compactions): builders read PLAN §0–§2, their own charge, the board row, GATE §0 and the glossary; the constitution only by the sections their charge names, never whole; scouts read the generated ledger report as their checklist and never the ledger source; after any compaction, re-read only the charge and the checklist. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.115 (2026-09-14) narrative, folded from the header:** THE PROTECTED MARK (owner: 'make sure that the things we iterated quite a bit and proved in manual sessions aren't being overwritten'): ledger rows that passed the walk or were owner-iterated in manual sessions carry protected=true; a fix packet changes only the flagged corner of a protected row, re-proven, never its behavior; the impact list and the walk put protected rows first; a regression there stops everything. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.116 (2026-09-15) narrative, folded from the header:** THE PALACE PACKAGE IS nocturne-memory (owner: 'I feel like to the uninitiated nocturne-memory is the most appropriate'; on 'spine': 'Spine has kind of become synonymous with ai slop'): the memory product publishes to PyPI as `nocturne-memory`; the harness stays `nocturne-ai` with the `nocturne` command (D.2 073); the repo and internal package may keep the spine name because the wheel name hides build architecture from the user (the org-chart rule, v2.48). Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.117 (2026-09-15) narrative, folded from the header:** NAMES MATCH (owner: 'I want it to be nocturne-harness and nocturne-memory'): the two packages on PyPI are `nocturne-harness` and `nocturne-memory`, and each repository carries the same name (nocturne-spine is renamed nocturne-memory; nocturne-harness is unchanged); the `nocturne` command stays. Supersedes D.2 073's `nocturne-ai` and v2.116's package-only rename. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.118 (2026-09-15) narrative, folded from the header:** THE SPRINT, AND MEMORIES-THEN-DROP IS THE DEFAULT (owner: 'I think we should do memories then drop as the default and get to work on this compaction thing. We should be using all our new review functionality mechanisms and be sprinting to complete all the rest of the features we have described by getting them fully spec'd and built asap. My current goal is to get to the next review day and be able to build something with this tech asap.'): the compaction suite (SD-061) is approved and MEMORIES-THEN-DROP is the default strategy; every unfinished ledger row (not built, partial, unverified) belongs to exactly one packet, fully spec'd at mint and walked at exit under the ledger, the impact bell, the protected mark and the rules machine; packets fire in waves (PLAN §5.0) and the sprint ends at the fourth walk, the review-day scout. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.119 (2026-09-15) narrative, folded from the header:** THE THREE 3D MODULES, AND ROOTS OPACITY IS A LOOK (owner: 'not have the full 3d multiview render but 3 different modules that are rendered seperately like we last said, but that being said they NEED to look really cool and be based on function and rendered in three js'; on roots opacity: 'just make it look really sick'): R10 (D.2 114b, the cube dissolves into modules) stands; the three literally-3D modules are FARM, ROOTS and PALACE, each rendered in three.js, each bound to real data (the pure-render law, v2.104), each held to the look law (steal the look, v2.113-era references); TIPS, LEDGER and SCORE remain the rack modules they already are. Roots opacity carries no value rule — it is derived by the look, not by a mapping. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).

**v2.120 (2026-09-16) narrative, folded from the header:** THE SPLIT IS A NOTE, NOT A WALL; THE PACKET RE-FREEZES (owner: 'Are we over constraining or am i being impatient. It's surprising that you are able to resolve all of these issues without me if we aren't over constraining.' — we were): a packet's declared file set is a coordination note; a file outside it is taken BY FUNCTION — only what the packet's rows need, in functions no peer's pushed branch touches — with a rebase on origin/main before the handoff push and the crossings listed in the handoff; STOP only for a same-function collision with a peer's live work. The mutexes are git (no force-push) and the CI coverage gate, not the file list. A packet that adds or resolves a flag or changes the ledger runs the freeze export and ships the refreshed frozen copies in its own product pushes; the gate no longer stands between a handoff and a green rules check. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).
