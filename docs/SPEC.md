...
**Version 2.114** (2026-09-13) — THE BOOT DIET (owner: 'was there a reason the compaction was weird?'; the gate's finding: the boot reading is ~940 KB, the constitution alone 468 KB; the rules-machine session re-read PLAN 20× and SPEC 13× after compactions): builders read PLAN §0–§2, their own charge, the board row, GATE §0 and the glossary; the constitution only by the sections their charge names, never whole; scouts read the generated ledger report as their checklist and never the ledger source; after any compaction, re-read only the charge and the checklist. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).
