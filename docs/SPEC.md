...
**Version 2.115** (2026-09-14) — THE PROTECTED MARK (owner: 'make sure that the things we iterated quite a bit and proved in manual sessions aren't being overwritten'): ledger rows that passed the walk or were owner-iterated in manual sessions carry protected=true; a fix packet changes only the flagged corner of a protected row, re-proven, never its behavior; the impact list and the walk put protected rows first; a regression there stops everything. Full version lineage: Appendix D.2 (rows) and D.3 (narrative).
