...
**Version 2.116** (2026-09-15) — THE PALACE PACKAGE IS nocturne-memory (owner: 'I feel like to the uninitiated nocturne-memory is the most appropriate'; on 'spine': 'Spine has kind of become synonymous with ai slop'): the memory product publishes to PyPI as `nocturne-memory`; the harness stays `nocturne-ai` with the `nocturne` command (D.2 073); the repo and internal package may keep the spine name because the wheel name hides build architecture from the user (the org-chart rule, v2.48). Full version lineage: Appendix D.2 (rows) and D.3 (narrative).
